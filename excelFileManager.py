...
# Date: 23-27 July, 30-31 July, 6-10 August, 17 August, 20-21 August 2018
#
...
#   milestone_label_column - any cell in the column containing the 'label'
#                            (e.g., 'A', B', 'C') for each milesteone;
#                            only the column index of this cell is used
#   milestone_list_first_row - any cell in the first row of the list
#                               of milestones; only the row index of this
#                               cell is used
#   milestone_name_column - any cell in the column containing the name
#                           of each milestone; only the column index of this
#                           cell is used
...
#
# dump_defined_names - Dumps all defined names found in input Excel file in
#                      human-readable format.
#
...
#                     and column index
#
# get_last_used_sched_column - returns index of the last column in the input Excel file
#                              that contains meaningful schedule data, i.e., has either
#                              been filled in with the MAGIC_FILL_STYLE and/or
#                              contains an uppercase letter indicating a milestone
#
#
...
# Dumps all defined names in the workbook inidcated by fullpath
# Ack: https://stackoverflow.com/questions/49508852/openpyxl-listing-all-defined-names-of-a-workbook
def dump_defined_names(fullpath):
    wb = openpyxl.load_workbook(fullpath, data_only=True)
    tree = wb.defined_names.to_tree()
    for ranges in tree:
	    print ranges.text + ' : ' + str(ranges.values())
    # end_for
# end_def dump_defined_names()
...
def initExcelFile(fullpath):
    ...
    try:
        retval['milestone_list_first_row_ix'] = get_row_index(wb, 'milestone_list_first_row')
    except:
        retval['errors'] += 'Failed to find defined name: milestone_list_first_row.\n'
    ...
    try:
        ...
        if maj_units == 'Quarter':
            ...
            # Error - undefined schedule major units
            retval['errors'] += 'Schedule major units not defined.'
            # Boilerplate values set, simply to allow execution to continue (even if erroneous)
            min_units = 'Undefined'
            num_subdivisions = 1
        # end_if
